...
bool identify_propagate_and_generate_gates(){
 ...
 for(int i = NN-2; i>0; i--){
   ...
   if(internal_xor->parents_size() == 3 && i < 3*cmp/4  && !cin_in_slice_0()){
   ...
   Gate * ix1 = xor_left_child(internal_xor);
   Gate * ix2 = xor_right_child(internal_xor);

   if(ix1->parents_size() !=2 && ix2->parents_size() !=2 &&
...
bool identify_final_stage_adder(){
  ...
  if(!identify_propagate_and_generate_gates()){
    msg("substitution not possible - not all propagate and generate gates found");
    return 0;
  }
  fix_inputs();
...
void add_original_adder(){
  for(unsigned i = 0; i < M-1; i++){
    ...
    if(n->get_fsa_inp() || n == carry_in){
      ...
      if(verbose >= 4) msg("original adder and %i %i %i", and1->lhs,
                                                          and1->rhs0,
                                                          and1->rhs1);
    }
  }
...
void fill_original_outputs(){
  ...
  if(c_out->get_aig_output()){
    ...
    else original_outputs.push_back(not_(c_out->get_var_num()));
...
void add_btor_adder(){
  ...
  if ((!signed_mult || booth) && cin_in_slice_0()){
    if(gate(slit(2))->parents_size() > 1){//sp-ba-bc
      inputs.pop_back();
      inputs.pop_back();
...
bool miter_to_file(FILE * file){
  ...
  refs = (unsigned*) calloc (2*(miter->maxvar+1), sizeof *refs);
  assert(refs);
  ...
  map = (int*) calloc (2*(miter->maxvar+1), sizeof *map);
  m = 0;
  n = 1;
  if (refs[0] || refs[1]) {
    map[0] = -1;
...
void generate_rewritten_aig(){
  ...
  for (unsigned i = 0; i < get_model_num_inputs(); i++) {
    aiger_add_input (rewritten, get_model_inputs_lit(i), get_model_inputs_name(i));
  }
  ...
  for(unsigned i = NN; i < M-1; i++){
    ...
    if((int) aiger_strip(and1->rhs0) == carry_out->get_var_num()){
      if (!aiger_sign(and1->rhs0)) out = not_(btor_carry);
      else out = btor_carry;
      aiger_add_and (rewritten, and1->lhs, out, and1->rhs1);
      if(verbose >= 4) msg("rewritten and %i %i %i", and1->lhs, out, and1->rhs1);
    }
    else if ((int) aiger_strip(and1->rhs1) == carry_out->get_var_num()){
      if (!aiger_sign(and1->rhs1)) out = not_(btor_carry);
      else out = btor_carry;
      aiger_add_and (rewritten, and1->lhs, out, and1->rhs0);
      if(verbose >= 4) msg("rewritten and %i %i %i", and1->lhs, out, and1->rhs0);
    }
    else {
